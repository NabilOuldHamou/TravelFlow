...
public class RandomSolution {
     ...
     * Each solution is built day-by-day ensuring:
     *  - The first and last nodes are the designated hotels.
     *  - Each day does not exceed the allowed travel distance.
     *  - Each site (non-hotel) is visited at most once.
     *
     * Only valid solutions (as per checkRoutesValidity) are added.
     *
     * @param n the number of valid solutions to generate.
     * @return a list of valid solution results.
     */
    public List<SolutionResult> generateSolutions(int n) {
        ...
        while (solutions.size() < n) {
            ...
            if (attempts < maxAttemptsPerSolution && routes != null) {
                ...
                // If after maxAttempts a valid solution was not found, break to avoid an infinite loop.
    ...
    private List<Route> generateSolutionRoutes() {
        ...
        for (int day = 0; day < instance.getNbrDays(); day++) {
            ...
            // Try adding sites while remaining distance allows.
            boolean addedSite;
            do {
                addedSite = false;
                // Get candidate sites that have not been visited and can be reached and allow returning to a hotel.
            ...
            // End the day by selecting a hotel.
            Hotel endHotel;
            if (lastDay) {
                ...
                if (reachableHotels.isEmpty()) {
                    // If no hotel is reachable, choose the nearest one.
    ...
    /**
     * Checks the validity of the list of routes according to the following constraints:
     *  - The first route starts at instance.getFirst() and the last route ends at instance.getLast().
     *  - The number of days is equal to instance.getNbrDays() (i.e. route count equals nbrDays).
     *  - Each route’s total distance does not exceed its allowed distance.
     *  - Non-hotel sites are visited at most once.
     */
